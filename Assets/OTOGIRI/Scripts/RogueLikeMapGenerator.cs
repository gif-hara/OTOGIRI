...
public class RogueLikeMapGenerator : MonoBehaviour
{
    ...
    private async UniTask Generate()
    {
        ...
        for (var i = 0; i < roomCount - 1; i++)
        {
            ...
            {
                var size = chunks[j].raw.width * chunks[j].raw.height;
                if(size > maxSize)
                {
                    maxSize = size;
                    targetIndex = j;
            ...
            var targetChunk = chunks[targetIndex];
        ...
        // 小さすぎる部屋は削除する
        chunks.RemoveAll(x => x.room.width < 4 || x.room.height < 4);
        ...
        foreach (var chunk in chunks)
        {
            foreach (var neighborChunk in chunks)
            {
